...
2.4G 按一次实测扫描选了台，当时的结论是 **ch6**：ch1 有 4 个同频 AP（最强
-62 dBm，正在用）、ch6 只有 1 个（-84 dBm）、ch11 自己没人但被夹在 ch9（-60）和
ch13（-53）之间、两侧各压掉一半带宽。**这个结论在第二天被推翻，见下面 09-15 的
信道一节**——写在这里是因为它值得留个记录：那天量出来的数没错，错在把一个会变的
东西当成了常量。5G 保持 **ch36 / VHT80** 不变：它所在的 80MHz 块（36–48）块内最强干扰只有
...
锐捷密码当时还停在那个从没通过门户验证的候选值上（`status` 只输出「位数 + 末
两位」而不输出明文，显示 11 位、末两位 `17`）。这笔账在下面 09-15 的密码一节结清。
...
边界是刻意划的。会断线或者会覆盖用户选择的修复不进默认路径：LAN 地址、WAN 协议、
Wi-Fi 密码和 2.4G/5G 信道只报告不修改，无线模式（HT20/VHT80）要 `--fix-wifi`，
门户重新认证要 `--fix-auth`。信道不在可修复项里不是漏了：射频环境会变，而写死的信道
在环境变了以后不会自己纠正，只会把人从好信道推到坏信道上（这条是 09-15 踩出来的，
见下面 09-15 的信道一节）。模式是另一类东西——HT20/VHT80 是网卡能力问题，跟邻居
无关，所以照旧强制。
...
夹具本身也是被测对象。清理 `/var/run/mwan3` 那段原本用的是 `rmdir`，而真机上这个
目录里必然有 `iface_state/`、`iptables_log/`、`mmx_mask`，`rmdir` 只会报
`Directory not empty` 然后放弃——修复从上线起就是摆设。它能一路绿到 09-15，是因为
夹具当年用 `mkdir` 造了个**空**目录，恰好是 `rmdir` 唯一删得掉的东西。现在夹具按真机
的样子造残留，`rmdir` 一改回去那条用例立刻变红（已做反事实验证）。
...
```sh
# 从开发机一条命令跑完（ssh 只自动尝试 id_rsa/id_ecdsa/id_ed25519，这台路由器的
# 钥匙不在其中，所以要 --ssh-key）
./scripts/acrh17-campus-up.sh --host root@192.168.5.1 --ssh-key ~/.ssh/id_acrh17 \
    fix --user 2021xxxxxxxx --password '<密码>' --password '<另一个候选>'
...
抄过来没用、链路一变就失效，所以只能在路由器上现场抓。`ruijie-refresh-query` 也从这同一个
302 里取 origin 写进 `server`（截到第一个 `/` 为止，端口保留，只认 http/https；`capture`
只抓不登录时同样写），刷机后不必再靠人手填。`reauth` 失败则两个字段成对回滚：只回滚 query
会留下「旧串配新门户」，下一次登录就发到一台不认识这个串的门户上。
...
密码默认不经过这个脚本，但 `--password` 可以给，而且可以重复给。候选密码不是直接写
进 UCI 的，而是逐个交给 `ruijie-password set`：它拿密码做一次真实登录，被门户拒绝就
把上一个值放回去。所以猜错一个候选不会留下一个坏密码，也不会让「密码对不对」变成
没人验证过的假设——第一个通过验证的留下，第二个才轮到。退出码 2（请求没送达门户，
或答复读不懂）意味着这次尝试没有产生任何关于密码的证据，那就不该拿它当拒绝，脚本
停在那里而不是接着猜下一个；75（另一个认证动作持锁）同理。已经在用的密码一概不动，
重设一个没被投诉过的密码只是白白断一次线。不给 `--password` 时脚本不碰密码。

这条链有个前因后果不能颠倒：`ruijie-password` 要求 `server` 与 `username` 已经在配置
里，否则请求根本发不出去，`last_reason` 判成 `unknown`，密码写入了却没有被验证。脚本
内已经把顺序排好（门户地址 → query_string → 学号 → 密码 → 开服务）；手动分三步做时
同样是先 `fix`、再 `ruijie-password set '<密码>'`、最后再跑一次 `fix`，缺凭据时脚本会
把这几步打出来。登录排在起守护进程之前也是同一个道理：守护进程只在真正发请求时拿
认证锁，先起服务会和这次登录抢锁，把一次本来能成的登录变成退出码 75。

`tests/test_campus_up.py` 52 项，夹具补上了 doctor 那份没有的几样东西：一份会说
`Location` 的 `curl`、一个真会按路径取值（而不是永远返回空）的 `jsonfilter`、一个
能表态在线与否的 `ruijie-auth`、一个按门户裁决和退出码表态的 `ruijie-password`。
桩的日志全部落在假根目录之外，所以「`check` 一个字节都没动」这条断言不会把自己的
日志当成脏改动。九个关键判据做了反事实验证——去掉门户页判据、去掉 `--interface`、
把登录挪到起服务之后、把 75 当成失败、允许没有密码就开服务、放行缺 `mac` 的 query、
把候选密码明文打进输出、把「没能验证」当成继续猜、重设一个已在用的密码，对应的用例
都会变红。

### 这条链路其实不做认证

上面那套东西是当作「路由器的有线口需要过 ePortal」来写的。09-15 现场核查下来，**这个
前提不成立**：SCAU 的有线口既不跑 ePortal 也不跑 802.1X，校园侧是按 MAC 放行的（用户
在校园网系统里给路由器开了「无感认证」）。证据是四条互相独立的观察：`wan` 上 12 秒
抓包 0 个 EAPOL 帧（挂 wpa_supplicant 是没用的）；`generate_204` 直接返回 204，没有
302，也就是根本没有强制门户；拿这套凭据去 `InterFace.do?method=login`，门户回的是
`设备未注册,请在 ePortal 上添加认证设备`——它认识这个报文，只是不认识这台机器；广播域
是平的，`wan` 上看得见 172.16.64.x–172.16.79.x 一整段别人的 ARP 与 mDNS。加上用户
确认的「09-13 23:40 起一直没掉线」，结论是：这台机器的出口靠的是它没变过的 WAN MAC
（`04:d9:f5:ea:3d:5c`）在白名单里，跟密码对不对无关。

前四条都是「它不需要认证」的旁证。第五条反过来做，主动去断它：在校园网系统里对着这台
设备点「下线」。点三次，逐秒探针抓到三次断线（探针每秒同时探一个裸 IP 和一个域名，
把「通不通」和「DNS 好不好」分开记），每次一秒上下。失败是**立刻返回**的连接被拒而不是
超时——`--max-time 3` 的两支探针要是真在等超时，那一条采样会拖到六秒以上，而实测间隔仍
是两秒。被主动拒绝，说明对面确实动了手，不是丢包。

值得注意的是断的这段时间路由器**什么都没有做**：`ruijie-auth` 没在跑，WAN 的
`carrier_changes` 从开机起一直是 1，接口 `uptime` 跨过整个事件没有重置，DHCP 租约与 IP
（`172.16.67.126`）都没变，`logread` 里一个字都没有。所以把它放回去的是校园侧。

这条顺带纠正了一个想当然的说法：「下线」按钮并不是形同虚设，它真的会掐断，只是掐不住。
放行跟着 MAC 走，会话一退掉，下一包流量就把它重新捞起来。**能用它触发一次重认证，但
没法用它把路由器踢下线**——改动动过出口之后，点一下下线、看它几秒内能不能自己回来，
是个不用跑到路由器旁边的验证手段。

所以 `campus-up.sh` 和 `ruijie-auth` 是**预案**，不是这台机器现在缺的东西。`ruijie.main.enabled`
保持关闭；doctor 关于门户状态文件的那条 WARN（状态文件写着 `authenticating`，而链路本身
早就通了）就是这个错位留下的痕迹，不是故障。

### 密码：把没验证过的那个值从回退路径上摘掉

密码换成了门户真正认的那个（`status` 现在只有一行「8 位、末两位 `As`」）。重点不在换，
在于**回退点里原来躺着的是那个从没通过验证的候选值**（11 位、末两位 `17`）：只要有人
在 LuCI 点「换回上一个密码」，就会被换到一个错的密码上，而且屏幕上看不出区别——回退
前后打印的都只是一个掩码。处理是让工具自己结账，不用手改 UCI：`ruijie-password drop`
丢回退点，`password_original` 置成空。空不是敷衍——这台机器刷完机就是没有密码的，
「最初密码」的真值本来就是空，`revert --original` 面对空值会明说「这台设备出厂时没有
配置过密码，没有可换回的值」，这正是该说的话。留一个错的锚点反而更糟：它会让下一次
`set` 不再重新锚定。

### 信道不能写死，无线模式可以

2.4G 前一天被定在 ch6（理由是同频干扰比 ch1 低 6 倍）。09-15 复扫三次，结论相反：
ch5 上一台 **-45 dBm** 的 `SCAUNET_1x` 把 ch6 压到 -44 dBm，而 ch1 是 -66 dBm，差
20 dB——路由器和它的客户端一直待在当天评估里最差的信道上。教训不是「那次量错了」，
是**射频环境会变，而写死的信道不会跟着变**。所以 doctor 现在一个字节都不写信道，
`--fix-wifi` 也不写，只强制 `htmode`：HT20/VHT80 属于网卡能力，跟邻居无关。

同一次检查还发现 radio1 根本没配 `htmode`，跑的是 `NOHT`——2.4G 压根没开 802.11n。
补上 `HT20` 后客户端协商从传统速率升到 72.2 MBit/s（MCS 7），并顺带停掉了一场持续
很久的 `AP-STA-POSSIBLE-PSK-MISMATCH` 刷屏（累计 504 条）。这类「配置里少一项」的
问题不会报错，只会安静地慢，所以 doctor 才要按能力值而不是按「有没有配」去比。

真机复跑：64 通过、1 警告（上面那条门户状态错位）、0 失败。
