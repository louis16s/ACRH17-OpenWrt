...
状态。「认证操作」区提供立即登录、注销认证、重新认证、重启认证服务、重新
获取 WAN DHCP、重新抓取 queryString、清除最近结果，以及验证当前密码、回退到
上一个密码和丢弃密码回退点。手动操作与后台登录共用一把锁，锁忙时不会误判为
认证失败。所有按钮都在后台以短超时执行，避免 LuCI 因门户超时而卡住；
